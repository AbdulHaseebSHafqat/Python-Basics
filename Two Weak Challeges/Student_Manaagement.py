class Add_student:
    def __init__(self, id, name , age, number):
        self.id = id
        self.name = name
        self.age = age
        self.number = number

    def view_data(self):
        print(f"{self.id}, {self.name}, {self.age}, {self.number}")


    def student_record(self):
        with open("Student_record.txt" , 'a') as file:
            file.write(f"{self.id}, {self.name}, {self.age}, {self.number}\n")

class Search_student:
    def search_student(student_id):
        with open("Student_record.txt" , 'r') as file:
            for lines in file:
                data = lines.strip().split(',')
                if student_id == data[0]:
                    print(data[0])
                    print(data[1])
                    print(data[2])
                    print(data[3])
                    return
        print("Search student successfully")

class update_student:
    def update_student(student_id , new_data):
        line = []
        with open("Student_record.txt", 'r')as file:
            for lines in file:
                data = lines.strip().split(',')
                if student_id == data[0]:
                    data[1] = new_data
                line.append(",".join(data) + "\n")
        with open("Student_record.txt" , 'w')as file:
            for lines in line:
                file.write(lines)

class delete_student:
    def delete_student(student_id):
        lines = []
        with open("Student_record.txt" , 'r') as file:
            for line in file:
                data = line.strip().split(',')
                if student_id == data[0]:
                    continue
                lines.append(",".join(data) + "\n")
        with open("Student_record.txt" , 'w') as file:
            for line in lines:
                file.write(line)
        print("Student deleted")

class add_course:
        def __init__(self, course_id, course_name, course_hour):
            self.course_id = course_id
            self.course_name = course_name
            self.course_hour = course_hour

        def Store_course(self):
            with open('Course_record.txt' , 'a') as file:
                file.write(f"{self.course_id} ,{self.course_name} , {self.course_hour} \n")

# class Enroll_students:
#     def enroll_students(student_id ,course_id):
#         student_found = False
#         course_found = False
#         with open("course_record.txt" , 'r') as file:
#             for line in file:
#                 data = line.strip().split(',')
#                 if course_id ==  data[0]:
#                     course_found = True
#                     break

#         with open("Student_record.txt" , 'r') as file:
#                     for line in file:
#                         data = line.strip().split(',')
#                         if student_id ==  data[0]:
#                             student_found = True
#                             break

#         if student_found and course_found:
#             with open("enrollment_record.txt" , 'a') as file:
#                 file.write(f"{student_id} , {course_id}\n")
#             print("Student enrolled successfully")

#         else:
#             print("Student or course not found")


class Enroll_students:

    def enroll_students(student_id, course_id):

        student_found = False
        course_found = False

        # Check course
        with open("Course_record.txt", "r") as file:

            for line in file:

                data = line.strip().split(",")

                if course_id == data[0].strip():
                    course_found = True
                    break

        # Check student
        with open("Student_record.txt", "r") as file:

            for line in file:

                data = line.strip().split(",")

                if student_id == data[0].strip():
                    student_found = True
                    break

        # Enroll
        if student_found and course_found:

            with open("Enrollment_record.txt", "a") as file:
                file.write(f"{student_id},{course_id}\n")

            print("Student enrolled successfully")

        else:
            print("Student or course not found")

class View_courses:
    def view_courses(student_id):
        with open("Enrollment_record.txt", "r") as file:        
            for line in file:
                data = line.strip().split(",")
                if student_id == data[0]:
                    course_id = data[1]
                    with open('Course_record.txt','r') as file:
                        for course_line in file:
                            course_data = course_line.strip().split(",")

                            if course_id == course_data[0].strip():
                                print(f"Course ID: {course_data[0].strip()}")
                                print(f"Course Name: {course_data[1].strip()}")
                                print(f"Credit Hours: {course_data[2].strip()}")
                                print("------------------")


class Calculate_result:
    def add_marks(student_id, course_id, marks):

        with open("Result_record.txt", "a") as file:
            file.write(f"{student_id},{course_id},{marks}\n")

        print("Marks added successfully")


    # 2. View student's results
    def view_result(student_id):

        with open("Result_record.txt", "r") as file:

            for line in file:

                data = line.strip().split(",")

                if student_id == data[0]:

                    print(f"Student ID: {data[0]}")
                    print(f"Course ID: {data[1]}")
                    print(f"Marks: {data[2]}")
                    print("--------------------")




    # 3. Calculate total and average
    def calculate_result(student_id):

        total = 0
        count = 0
        with open("Result_record.txt" , 'r') as file:
            for line in file:
                data = line.strip().split(',')
                if student_id == data[0]:
                    marks = int(data[2])
                    total = total+marks
                    count = count + 1
        if count > 0:

            average = total / count

            print(f"Total Marks: {total}")
            print(f"Average: {average}")

            if average >= 80:
                print("Grade: A")

            elif average >= 70:
                print("Grade: B")

            elif average >= 60:
                print("Grade: C")

            elif average >= 50:
                print("Grade: D")

            else:
                print("Grade: F")

        else:
            print("No result found")






# s1 = Add_student(101, "Haseeb", 18, 3121312)
# s2 = Add_student(102, "Atiq", 19, 312)
# s3 = Add_student(103, "hassan", 20, 112)
# s4 = Add_student(104, "hassan", 20, 112)
# s1.view_data()
# s1.student_record()
# s2.student_record()
# s3.student_record()
# s4.student_record()
# c1 = add_course(22 , "English" , 2400)
# c2 = add_course(24 , "Urdu" , 200)
# c1.Store_course()
# c2.Store_course()
# Enroll_students.enroll_students('101', '22')

# Search_student.search_student("101")
# # Search_student.search_student("102")
# update_student.update_student("101", "ABdullah")
# delete_student.delete_student("103")
# # View_courses.view_courses("101")
# Calculate_result.add_marks('101', '22', 200)
# # Calculate_result.view_result('101')
# Calculate_result.add_marks("101", "22", 85)
# Calculate_result.add_marks("101", "24", 72)
# Calculate_result.view_result("101")
# Calculate_result.calculate_result("101")

while True:

    print("\n================================")
    print("    STUDENT MANAGEMENT SYSTEM")
    print("================================")

    print("1. Add Student")
    print("2. View Students")
    print("3. Search Student")
    print("4. Update Student")
    print("5. Delete Student")
    print("6. Add Course")
    print("7. Enroll Student")
    print("8. View Student Courses")
    print("9. Calculate Result")
    print("10. Exit")

    choice = input("\nEnter your choice: ")


    # --------------------------------------
    # 1. Add Student
    # --------------------------------------

    if choice == "1":

        student_id = input("Enter Student ID: ")
        name = input("Enter Name: ")
        age = input("Enter Age: ")
        number = input("Enter Phone Number: ")

        student = Add_student(
            student_id,
            name,
            age,
            number
        )

        student.student_record()

        print("Student added successfully.")


    # --------------------------------------
    # 2. View Students
    # --------------------------------------

    elif choice == "2":
        with open("Student_record.txt", "r") as file:
            for line in file:
                data = line.strip().split(",")

                print(f"ID: {data[0]}")
                print(f"Name: {data[1]}")
                print(f"Age: {data[2]}")
                print(f"Phone: {data[3]}")
                print("--------------------")


    # --------------------------------------
    # 3. Search Student
    # --------------------------------------

    elif choice == "3":

        student_id = input("Enter Student ID: ")

        Search_student.search_student(student_id)


    # --------------------------------------
    # 4. Update Student
    # --------------------------------------

    elif choice == "4":

        student_id = input("Enter Student ID: ")
        new_name = input("Enter New Name: ")

        update_student.update_student(
            student_id,
            new_name
        )


    # --------------------------------------
    # 5. Delete Student
    # --------------------------------------

    elif choice == "5":

        student_id = input("Enter Student ID: ")

        delete_student.delete_student(
            student_id
        )


    # --------------------------------------
    # 6. Add Course
    # --------------------------------------

    elif choice == "6":

        course_id = input("Enter Course ID: ")
        course_name = input("Enter Course Name: ")
        course_hour = input("Enter Credit Hours: ")

        course = add_course(
            course_id,
            course_name,
            course_hour
        )

        course.Store_course()

        print("Course added successfully.")


    # --------------------------------------
    # 7. Enroll Student
    # --------------------------------------

    elif choice == "7":

        student_id = input("Enter Student ID: ")
        course_id = input("Enter Course ID: ")

        Enroll_students.enroll_students(
            student_id,
            course_id
        )


    # --------------------------------------
    # 8. View Student Courses
    # --------------------------------------

    elif choice == "8":

        student_id = input("Enter Student ID: ")

        View_courses.view_courses(
            student_id
        )


    # --------------------------------------
    # 9. Calculate Result
    # --------------------------------------

    elif choice == "9":

        student_id = input("Enter Student ID: ")

        print("\n1. Add Marks")
        print("2. View Result")
        print("3. Calculate Result")

        result_choice = input("Enter choice: ")


        if result_choice == "1":

            course_id = input("Enter Course ID: ")
            marks = input("Enter Marks: ")

            Calculate_result.add_marks(
                student_id,
                course_id,
                marks
            )


        elif result_choice == "2":

            Calculate_result.view_result(
                student_id
            )


        elif result_choice == "3":

            Calculate_result.calculate_result(
                student_id
            )


        else:

            print("Invalid result option.")


    # --------------------------------------
    # 10. Exit
    # --------------------------------------

    elif choice == "10":

        print("Program ended.")
        break


    # --------------------------------------
    # Invalid Choice
    # --------------------------------------

    else:

        print("Invalid choice. Please try again.")